...
def  main():
  ...
  installer = "\installer\PluckMatrix Installer.exe"
   
  if demo:
    installer = "\installer\PluckMatrix Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\PluckMatrix manual.pdf" 
  ]
  ...
  ZIPNAME = "PluckMatrix-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "PluckMatrix-v" + FULLVERSIONSTR + "-win-demo.zip"
