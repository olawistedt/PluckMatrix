/* Declares the PluckMatrix Audio Worklet Node */

class PluckMatrixController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "PluckMatrix", options);
  }
  ...
  static importScripts (actx) {
    ...
    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/PluckMatrix-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/PluckMatrix-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
